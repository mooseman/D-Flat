...
// extern DBOX G_Syntax;
// extern DBOX Test;

char DFlatApplication[] = "MemoPad";
char teststrng[12]="12345.67e8";
...
// static int G_SyntaxProc(WINDOW,MESSAGE, PARAM, PARAM);
                ...
                case ID_SAVEAS:
                    ...
                    return TRUE;
                                case ID_PRINTSETUP:
                                        ...
                                        return TRUE;
                case ID_TEST:
                        DialogBox(wnd, &Test, TRUE, TestProc);
                        return TRUE;

//                case ID_G_Syntax:
//								DialogBox(wnd, &G_Syntax, TRUE, G_SyntaxProc);
//                        return TRUE;
...
static int G_SyntaxProc(WINDOW wnd, MESSAGE msg, PARAM p1, PARAM p2)
{             
        int rtn;
        double Tempvalue;
        struct MinMaxValue TempMinMax;
        char Tempstr[80], *endptr, test[20];
		  WINDOW cwnd;
        DBOX *db;
       CTLWINDOW *ct;

     strcpy(test,"Hallo Welt");

     switch (msg)
     {
     CREATE_WINDOW:
          rtn = DefaultWndProc(wnd, msg, p1, p2);
          PutItemText(wnd, ID_AUSGABE, "hallo welt");
...
testaufruf (WINDOW wnd)
    {
    char text[20];
    strcpy(text,"Aufruf aus der Procedur");
//    PutItemText(wnd,ID_AUSGABE,text);
    sprintf(wnd->text,"%c",text);
    SendMessage(wnd, PAINT, 0, 0);
